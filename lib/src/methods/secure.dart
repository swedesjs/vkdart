...
class Secure {
  ...
  Future<ApiResponse<T>> addAppEvent<T>(MethodTypeParams params) =>
      _api.request<T>('secure.addAppEvent', params);

  /// Checks the user authentication in 'IFrame' and 'Flash' apps using the 'access_token' parameter.
  Future<ApiResponse<T>> checkToken<T>(MethodTypeParams params) =>
      _api.request<T>('secure.checkToken', params);

  /// Returns payment balance of the application in hundredth of a vote.
  Future<ApiResponse<T>> getAppBalance<T>(MethodTypeParams params) =>
  ...
  Future<ApiResponse<T>> getSMSHistory<T>(MethodTypeParams params) =>
      _api.request<T>('secure.getSMSHistory', params);

  /// Shows history of votes transaction between users and the application.
  Future<ApiResponse<T>> getTransactionsHistory<T>(MethodTypeParams params) =>
      _api.request<T>('secure.getTransactionsHistory', params);

  /// Returns one of the previously set game levels of one or more users in the application.
  Future<ApiResponse<T>> getUserLevel<T>(MethodTypeParams params) =>
      _api.request<T>('secure.getUserLevel', params);

  /// Opens the game achievement and gives the user a sticker
  Future<ApiResponse<T>> giveEventSticker<T>(MethodTypeParams params) =>
      _api.request<T>('secure.giveEventSticker', params);

  /// Sends notification to the user.
  Future<ApiResponse<T>> sendNotification<T>(MethodTypeParams params) =>
      _api.request<T>('secure.sendNotification', params);

  /// Sends 'SMS' notification to a user's mobile device.
  Future<ApiResponse<T>> sendSMSNotification<T>(MethodTypeParams params) =>
      _api.request<T>('secure.sendSMSNotification', params);

  /// Sets a counter which is shown to the user in bold in the left menu.
  Future<ApiResponse<T>> setCounter<T>(MethodTypeParams params) =>
  ...
  Future<ApiResponse<T>> setUserLevel<T>(MethodTypeParams params) =>
